...
class StmMap<K,V> implements OurMap<K,V> {
  ...
  public V get(K k) {
    return atomic(() -> {
      ...
      ItemNode<K,V> first = bs[hash].get();
      ItemNode.search(first, k, old);
      return old.get();
    });
  }
  ...
  public V putIfAbsent(K k, V v) {
    return atomic(() -> {
      ...
      final int hash = h % bs.length;
      final ItemNode<K,V> node = bs[hash].get();
      boolean first = ItemNode.search(node, k, old);
      if(!first){
        bs[hash] = newTxnRef(new ItemNode<>(k, v, node));
  ...
  public V remove(K k) {
    return atomic(() -> {
      ...
      TxnRef<ItemNode<K,V>>[] bs = buckets.get();
      final int hash = h % bs.length;
      final ItemNode<K,V> first = bs[hash].get();
      ItemNode<K,V> node = ItemNode.delete(first, k, old);
      if(old.get() != null){
        bs[hash] = newTxnRef(node);
        // cachedSize.set(cachedSize.get() - 1);
        cachedSize.atomicIncrementAndGet(-1);
      }
      return old.get();
  ...
  public void forEach(Consumer<K,V> consumer) {
    ...
    for (int i = 0; i < bs.length; i++) {
      final int j = i;
      ItemNode<K,V> first = atomic(() -> bs[j].get());
      while(first != null){
        consumer.accept(first.k, first.v);
        first = first.next);
      }
    }
  }
