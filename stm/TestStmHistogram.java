...
class TestStmHistogram {
  ...
  private static void countPrimeFactorsWithStmHistogram() {
    final int SPAN = 30;
    final Histogram histogram = new CasHistogram(SPAN);
    final Histogram total = new CasHistogram(SPAN);
...
class StmHistogram implements Histogram {
  ...
  public void increment(int bin) {
    atomic(() -> counts[bin].set(counts[bin].get() + 1));
  }
  ...
  public int[] getBins() {
    return atomic(() -> {
      ...
      for (int i = 0; i < counts.length; i++) { arr[i] = getCount(i); }
      return arr;
    });
  }
  ...
  public void transferBins(Histogram hist) {
    for (int i = 0; i < counts.length; i++) {
      final int j = i;
      atomic(() -> {
        counts[j].set(counts[j].get() + hist.getAndClear(j));
      });
    }
  }
}
