...
class StmMap<K,V> implements OurMap<K,V> {
  ...
  // private final AtomicInteger cachedSize; 
  ...
  public StmMap(int bucketCount) {
    ...
    // this.cachedSize = new AtomicInteger();
  }
  ...
  public V get(K k) {
    ...
    atomic(() -> {
      ...
      ItemNode.search(bs[hash].get(), k, old);
    });
    return old.get();
  }
  ...
  public V put(K k, V v) {
    ...
    atomic (() -> {
      final TxnRef<ItemNode<K,V>>[] bs = buckets.get();
      final int h = getHash(k), hash = h % buckets.get().length;
      final ItemNode<K, V> node = bs[hash].get(), newNode = ItemNode.delete(node, k, old);
  ...
  public V remove(K k) {
    ...
    atomic ( () -> {
      ...
      ItemNode<K, V> n = ItemNode.delete(node, k, old);
      if(old.get() != null) {
        bs[hash] = newTxnRef(n);
        cachedSize.atomicIncrementAndGet(-1);
      }
  ...
  public void forEach(Consumer<K,V> consumer) {
    ...
    for (int hash=0; hash < length; hash++) { 
      final int i = hash;
      ItemNode<K,V> node = atomic(() -> bs[i].get());
      while (node != null) {
