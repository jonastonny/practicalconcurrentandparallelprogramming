...
public class TestFetchWebGui {
  ...
  private static void goodFetch() {
    ...
    ArrayList<DownloadWorker> al = new ArrayList<>();
    ...
    fetchButton.addActionListener(new ActionListener() {
      public void actionPerformed(ActionEvent e) {
        al.clear(); progressBar.setValue(0); count.set(0); // Reset Workers and progress! :) 
        for (String url : urls) {
          DownloadWorker downloadTask = new DownloadWorker(textArea, url, count);
          al.add(downloadTask);
        ...
        for (DownloadWorker downloadTask : al) {
  ...
  static class DownloadWorker extends SwingWorker<String,String> {
    ...
    public DownloadWorker(TextArea textArea, String url, AtomicInteger count) {
    ...
    public String doInBackground() {
      ...
      // for (String url : urls) {
        ...
        // sb.append(result); // (1)
        if (isCancelled()) return sb.toString(); 
        setProgress((100 * count.incrementAndGet()) / urls.length); // (2)
        publish(result); // (4)
      // }
      return sb.toString();
    }
    ...
    public void done() {
      ...
      } catch (InterruptedException exn) { exn.getCause(); }
